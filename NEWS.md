## 17.5.2 2026-09-04
* Review logs and add DB indexes (CIRCSTORE-674)

## 17.5.1 2026-07-29
* Fix intermittent issue with closed requests cannot be viewed (CIRCSTORE-624)

## 17.5.0 2026-04-14
* Update FQM translations (MODFQMMGR-1095)
* Add request view for FQM (MODFQMMGR-770)
* Revert unnecessary changes related to adding an index to request.itemId (CIRCSTORE-643)
* Set an ID column in actual-cost-record (MODFQMMGR-1086)
* Enable GitHub workflow for Maven (CIRCSTORE-641)
* Remove unnecessary field overrides from config (MODFQMMGR-1089)
* Add index to `request.itemId` (CIRCSTORE-643)
* Update actual cost records for FQM (MODFQMMGR-1086)
* Implement cache for locations and service-points API calls (CIRCSTORE-642)
* Lokalise: Translations update
* Publish actual_cost_record data via FQM (MODFQMMGR-1075)
* Flush and close Kafka producer (CIRCSTORE-639)
* Avoid request updates when nothing was changed (CIRCSTORE-632)
* Fix 400 error for canceling a request with a nonexistent cancellationReasonId (CIRCSTORE-631)
* Upgrade to Vert.x 5.0 (CIRCSTORE-608)
* Store request anonymization settings in circulation_sets (CICRSTORE-629)
* Add domain events for expired requests (CIRCSTORE-623)
* Migrate circulation settings (CIRCSTORE-596)
* Add hold trigger to notice enum (CIRCSTOR-620)
* Add RAML and JSON for requests anonymization (CIRCSTORE-604)
* Fix intermittent test failures related to circulation rules (CIRCSTORE-601)
* Update `item-storage` interface version (CIRCSTORE-599)
* Add `forUseAtLocation` field to loan-policy (CIRCSTORE-578)
* Allow to save loan policy with 0 duration (CIRCSTORE-595)
* Upgrade testcontainers to 1.21.0, apache/kafka-native to 3.8.0 (CIRCSTORE-592)
